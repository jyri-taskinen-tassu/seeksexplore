...
type InfoRow = { language: string; name: string | null; description: string | null; url: string | null; webshop_url: string | null };
type TagRow = { tag: string };
type AvailRow = { start_date: string | null; end_date: string | null; start_time: string | null; end_time: string | null; nr_of_tickets: number | null };

const MONTH_ORDER = ["january","february","march","april","may","june","july","august","september","october","november","december"];
const LANG_LABELS: Record<string, string> = { en: "English", fi: "Finnish", sv: "Swedish", de: "German", fr: "French", ru: "Russian", zh: "中文", ja: "日本語", ko: "한국어", es: "Español", it: "Italiano", nl: "Nederlands" };
const PRICING_UNITS = ["person", "hour", "day", "week"];
const PRODUCT_TYPES = ["accommodation","attraction","event","experience","rental_service","restaurant","shop","transportation","venue"];
...
function SectionCard({ id, title, children }: { id: string; title: string; children: React.ReactNode }) {
  return (
    <div id={`section-${id}`} className="rounded-xl border border-neutral-200 bg-white shadow-sm scroll-mt-20">
...
export default function ProductEditForm({ product }: { product: Record<string, unknown> }) {
  ...
  const initialTags = ((product.product_tags as TagRow[]) ?? []).map((t) => t.tag);
  const initialMonths = (product.available_months as string[] | null) ?? [];

  const [type, setType] = useState((product.type as string) ?? "experience");
  const [accessible, setAccessible] = useState((product.accessible as boolean) ?? false);
  const [infos, setInfos] = useState<InfoRow[]>(initialInfos);
  const [priceFrom, setPriceFrom] = useState<string>(product.price_from != null ? String(product.price_from) : "");
  const [priceTo, setPriceTo] = useState<string>(product.price_to != null ? String(product.price_to) : "");
  const [pricingUnit, setPricingUnit] = useState<string>((product.pricing_unit as string) ?? "person");
  const [durationDays, setDurationDays] = useState<string>(product.duration_days != null ? String(product.duration_days) : "");
  const [durationHours, setDurationHours] = useState<string>(product.duration_hours != null ? String(product.duration_hours) : "");
  const [durationMinutes, setDurationMinutes] = useState<string>(product.duration_minutes != null ? String(product.duration_minutes) : "");
  const [capacityMin, setCapacityMin] = useState<string>(product.capacity_min != null ? String(product.capacity_min) : "");
  const [capacityMax, setCapacityMax] = useState<string>(product.capacity_max != null ? String(product.capacity_max) : "");
  ...
  function updateInfo(lang: string, field: keyof InfoRow, value: string) {
    markDirty();
    setInfos((prev) => prev.map((i) => i.language === lang ? { ...i, [field]: value } : i));
  }
  ...
  async function handleSave() {
    setSaving(true);
    try {
      const res = await fetch(`/api/provider/products/${product.id as string}`, {
        method: "PATCH",
  ...
  const enName = infos.find((i) => i.language === "en")?.name ?? (product.type as string);
      ...
      <header className="flex-shrink-0 z-30 border-b border-neutral-200 bg-white">
        <div className="flex items-center justify-between px-6 py-3">
          <div className="flex items-center gap-3">
            <Link href={`/provider/products/${product.id as string}`} className="rounded-lg border border-neutral-200 p-1.5 text-neutral-500 hover:bg-neutral-50 hover:text-neutral-900 transition-colors">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M19 12H5M12 5l-7 7 7 7" />
              </svg>
            </Link>
            <div>
              <div className="text-sm font-semibold text-neutral-900 truncate max-w-[320px]">{enName}</div>
          ...
          <div className="mx-auto max-w-2xl px-6 py-6 space-y-6">
            ...
            <SectionCard id="info" title="Product info">
              <div className="space-y-4">
                <div>
                  <Label>Type</Label>
                  <Select value={type} onChange={(e) => { setType(e.target.value); markDirty(); }}>
                    {PRODUCT_TYPES.map((t) => (
                ...
                <div className="flex items-center justify-between rounded-lg border border-neutral-200 bg-neutral-50 px-4 py-3">
                  <div>
                    <div className="text-sm font-medium text-neutral-900">Accessible</div>
                    <div className="text-xs text-neutral-500">Product is accessible for people with disabilities</div>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" checked={accessible} onChange={(e) => { setAccessible(e.target.checked); markDirty(); }} className="sr-only peer" />
            ...
            <SectionCard id="languages" title="Languages">
              {availLangs.length === 0 ? (
                <p className="text-sm text-neutral-400 italic">No multilingual content imported.</p>
              ) : (
                <div>
                  {/* Lang picker */}
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => updateInfo(activeLang, "name", e.target.value)}
                      ...
                      <div>
                        ...
                        <Textarea
                          rows={6}
                          value={activeInfo.description ?? ""}
                          onChange={(e) => updateInfo(activeLang, "description", e.target.value)}
                          placeholder={`Description in ${LANG_LABELS[activeLang] ?? activeLang}`}
                        />
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) => updateInfo(activeLang, "url", e.target.value)}
                            placeholder="https://..."
                          />
                        </div>
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) => updateInfo(activeLang, "webshop_url", e.target.value)}
                            placeholder="https://..."
                          />
                        </div>
                      </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    ...
                    <Input
                      ...
                      step="0.01"
                      value={priceFrom}
                      onChange={(e) => { setPriceFrom(e.target.value); markDirty(); }}
                      placeholder="0"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      step="0.01"
                      value={priceTo}
                      onChange={(e) => { setPriceTo(e.target.value); markDirty(); }}
                      placeholder="Optional"
                    />
                  </div>
                  <div>
                    <Label>Unit</Label>
                    <Select value={pricingUnit} onChange={(e) => { setPricingUnit(e.target.value); markDirty(); }}>
                      {PRICING_UNITS.map((u) => (
                ...
                {priceFrom && (
                  <div className="rounded-lg bg-neutral-50 border border-neutral-200 px-4 py-3 text-sm text-neutral-600">
                    Preview: <span className="font-semibold text-neutral-900">
                      From €{priceFrom}{priceTo ? `–€${priceTo}` : ""} / {pricingUnit}
                    </span>
                  </div>
                )}
              </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="relative">
                      <Input
                        ...
                        min="0"
                        value={durationDays}
                        onChange={(e) => { setDurationDays(e.target.value); markDirty(); }}
                        placeholder="0"
                      />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-neutral-400 pointer-events-none">days</span>
                    </div>
                    <div className="relative">
                      <Input
                        type="number"
                        ...
                        placeholder="0"
                      />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-neutral-400 pointer-events-none">hrs</span>
                    </div>
                    <div className="relative">
                      <Input
                        type="number"
                        ...
                        onChange={(e) => { setDurationMinutes(e.target.value); markDirty(); }}
                        placeholder="0"
                      />
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="relative">
                      <Input
                        ...
                        min="1"
                        value={capacityMin}
                        onChange={(e) => { setCapacityMin(e.target.value); markDirty(); }}
                        placeholder="Min"
                      />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-neutral-400 pointer-events-none">min</span>
                    </div>
                    <div className="relative">
                      <Input
                        ...
                        min="1"
                        value={capacityMax}
                        onChange={(e) => { setCapacityMax(e.target.value); markDirty(); }}
                        placeholder="Max"
                      />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-neutral-400 pointer-events-none">max</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <SectionCard id="season" title="Season">
              <p className="text-xs text-neutral-500 mb-3">Select the months when this product is available.</p>
              ...
              {months.length > 0 && (
                <p className="mt-3 text-xs text-neutral-500">
                  Active: <span className="font-medium text-neutral-900">{months.sort((a, b) => MONTH_ORDER.indexOf(a) - MONTH_ORDER.indexOf(b)).map(m => m.slice(0,3)).join(", ")}</span>
                </p>
              )}
            </SectionCard>
            ...
            <SectionCard id="tags" title="Tags">
              <div className="flex flex-wrap gap-2 mb-3">
                {tags.map((t) => (
                  <span key={t} className="inline-flex items-center gap-1 rounded-lg border border-neutral-200 bg-neutral-50 px-3 py-1.5 text-xs font-medium text-neutral-700">
                    {t.replace(/_/g, " ")}
                    <button
                      type="button"
                      onClick={() => removeTag(t)}
                ...
                {tags.length === 0 && <p className="text-sm text-neutral-400 italic">No tags.</p>}
              </div>
