...
ALTER TABLE seeks_and_explore_demo.sales_opportunities
ADD COLUMN IF NOT EXISTS position INTEGER NOT NULL DEFAULT 0;

-- Backfill: assign per-stage positions ordered by created_at
UPDATE seeks_and_explore_demo.sales_opportunities so
SET position = sub.rn - 1
FROM (
  SELECT id, ROW_NUMBER() OVER (PARTITION BY stage ORDER BY created_at ASC) - 1 AS rn
  FROM seeks_and_explore_demo.sales_opportunities
) sub
WHERE so.id = sub.id;
